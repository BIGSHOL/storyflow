...
#    X __tests__/subscription/PlanBadge.test.tsx
#    O /path/to/worktree/phase-1-subscription/__tests__/subscription/PlanBadge.test.tsx
```
...
워크트리에서 작업을 시작합니다.
   - 대상 파일: __tests__/subscription/useSubscription.test.ts
   - Mock 파일: __mocks__/subscriptionService.ts
```
...
- Supabase Test Helpers (DB 테스트)

책임:
1. 프론트엔드 컴포넌트에 대한 유닛 테스트 작성
2. 커스텀 훅에 대한 테스트 작성
3. Supabase API 모킹
4. E2E 테스트 시나리오 구현
...
출력:
- 프론트엔드 테스트 파일 (`__tests__/**/*.test.tsx`)
- 훅 테스트 파일 (`__tests__/hooks/*.test.ts`)
- Mock 파일 (`__mocks__/*.ts`)
- E2E 테스트 (`e2e/*.spec.ts`)
- 테스트 설정 파일 (`vitest.config.ts`)
- 테스트 커버리지 요약 보고서
